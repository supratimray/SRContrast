...
NSString *SRCXTAutosaveKey = @"SRCXTAutosave";

@implementation SRCXTController
...
- (id)init {
    ...
    if ((self = [super initWithWindowNibName:@"SRCXTController"]) != nil) {
 		[self setShouldCascadeWindows:NO];
        [self setWindowFrameAutosaveName:SRCXTAutosaveKey];
...
- (void)windowDidLoad {
 ...
 //   [xtView setSamplePeriodMS:kSamplePeriodMS spikeChannels:kSpikeChannels spikeTickPerMS:kTimestampTickMS];
	...
	[[self window] setFrameUsingName:SRCXTAutosaveKey];			// Needed when opened a second time
...
- (void)preStimuli:(NSData *)eventData eventTime:(NSNumber *)eventTime {
    ...
    [xtView eventName:@"PreStimulus" eventTime:eventTime];	
}

- (void)visualStimsOn:(NSData *)eventData eventTime:(NSNumber *)eventTime {
    [xtView eventName:@"Stims On" eventTime:eventTime];
}
...
- (void)stimulus:(NSData *)eventData eventTime:(NSNumber *)eventTime {
	...
	//NSLog(@"GaborOn:%d", [eventTime longValue]);
...
- (void)stimulusOff:(NSData *)eventData eventTime:(NSNumber *)eventTime {
	...
	[xtView stimulusBarColor:[NSColor whiteColor] eventTime:eventTime];
	//NSLog(@"GaborOff:%d", [eventTime longValue]);
